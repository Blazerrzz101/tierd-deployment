// Script to fix vote function parameter order
...
async function applyVoteFix() {
  console.log('Applying vote function fix...');

  // The SQL from the 20250226000000_fix_vote_function_signature.sql file
  const sql = `
...
GRANT EXECUTE ON FUNCTION public.has_user_voted(uuid, uuid, text) TO authenticated, anon;`;
  ...
  try {
    ...
    console.log('Vote function fix applied successfully!');
    return true;
  } catch (error) {
...
if (require.main === module) {
  applyVoteFix()
    .then(success => {
      if (success) {
        console.log('Vote function migration completed successfully.');
        process.exit(0);
      } else {
        console.error('Vote function migration failed.');
        process.exit(1);
      }
    })
    .catch(error => {
