-- Function to validate and process votes with enhanced auth checks
    ...
    IF p_anonymous_id IS NOT NULL THEN
        ...
        VALUES (
            ...
            jsonb_build_object(
                ...
                'ip_hash', encode(digest(current_setting('request.headers')::json->>'x-real-ip', 'sha256'), 'hex')
            )
        )
        RETURNING jsonb_build_object(
            'success', true,
...
-- Create function to get user's votes
...
BEGIN
    ...
    ELSE
        -- Get anonymous votes based on client ID
        v_anonymous_id := current_setting('request.headers')::json->>'x-anonymous-id';
        ...
        IF v_anonymous_id IS NOT NULL THEN
            ...
            SELECT 
                ...
                v.created_at
            FROM public.votes v
            WHERE v.metadata->>'anonymous_id' = v_anonymous_id
