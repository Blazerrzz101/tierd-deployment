...
-- Check if the threads table exists and has a title column
DO $$ 
BEGIN
    -- Only proceed if the table exists and has a title column
    IF EXISTS (
        SELECT 1 
        ...
        -- Fix any null titles in existing threads
        UPDATE public.threads
        SET title = 'Untitled Thread'
        WHERE title IS NULL;
