-- Add url_slug column to products table without NOT NULL constraint first
ALTER TABLE public.products
ADD COLUMN url_slug TEXT;

-- Make url_slug unique
...
-- Drop and recreate the product_rankings materialized view
...
SELECT 
    ...
    0 as net_score,
    ROW_NUMBER() OVER (PARTITION BY p.category ORDER BY RANDOM()) as rank
FROM public.products p;

-- Create a unique index on product_id for the materialized view
CREATE UNIQUE INDEX product_rankings_product_id_idx ON public.product_rankings (product_id);

-- Now add the NOT NULL constraint after data is populated
ALTER TABLE public.products
