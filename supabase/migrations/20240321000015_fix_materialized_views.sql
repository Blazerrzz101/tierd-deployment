...
-- Recreate materialized views with proper type casting
...
SELECT 
    ...
    SUM(CASE WHEN v.vote_type::integer = 1 THEN 1 ELSE 0 END) as total_upvotes,
    SUM(CASE WHEN v.vote_type::integer = -1 THEN 1 ELSE 0 END) as total_downvotes
FROM products p
LEFT JOIN votes v ON p.id = v.product_id
GROUP BY p.category;
