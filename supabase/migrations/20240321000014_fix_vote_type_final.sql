-- Drop dependent views and policies first
DROP MATERIALIZED VIEW IF EXISTS product_rankings;
DROP MATERIALIZED VIEW IF EXISTS category_stats;
DROP POLICY IF EXISTS "Anonymous users can vote with rate limiting" ON votes;
...
ALTER TABLE public.votes ALTER COLUMN vote_type TYPE integer USING 
    CASE 
...
-- Create policy for anonymous voting
...
WITH CHECK (
    ...
    (
        SELECT COUNT(*)
        FROM votes v
        WHERE v.metadata->>'client_id' = metadata->>'client_id'
